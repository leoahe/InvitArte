...
if (noviaFooter) noviaFooter.textContent = document.getElementById('nombre-novia')?.textContent || 'Camille';
if (novioFooter) novioFooter.textContent = document.getElementById('nombre-novio')?.textContent || 'Antoine';
...
const wrap = document.getElementById('ji-particles');
if(wrap){
  for(let i=0;i<30;i++){
    const el=document.createElement('div');
    el.className = i<18 ? 'ji-leaf' : 'ji-gold-dot';
...
/* ── RSVP ── */
window.rsvpJI=(resp,btn)=>{
  const msg=document.getElementById('rsvp-msg');
  document.querySelectorAll('.ji-rsvp-btn').forEach(b=>b.disabled=true);
  if(resp==='si'){btn.style.background='var(--ji-dorado)';msg.textContent='¡Gracias! Te esperamos con mucho cariño. ✦';}
